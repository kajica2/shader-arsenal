...
void main() {
  ...
  vec3 color1 = vec3(0.0, 0.7, 1.0) * glow1;
  vec3 color2 = vec3(1.0, 0.3, 0.2) * glow2;
...
void main() {
  ...
  float pulse = sin(a * 10.0 + t * 2.0) * 0.05 * u_mid;
  float d = abs(r - (0.4 * u_zoom) - pulse);
  float glow = 0.01 / (d + 0.008);
  
  vec3 col = mix(vec3(0.0, 0.8, 1.0), vec3(1.0, 0.0, 0.5), sin(a + t) * 0.5 + 0.5) * glow;
  col += vec3(1.0, 1.0, 1.0) * smoothstep(0.01, 0.0, d) * u_bass * 0.5;
...
void main() {
  ...
  float grid = sin(a * 8.0) * sin(depth * u_zoom - t * 4.0);
  grid = smoothstep(0.2, 0.9, grid);
  
  vec3 col = mix(vec3(0.5, 0.0, 1.0), vec3(0.0, 1.0, 0.8), sin(depth * 0.2 + t) * 0.5 + 0.5);
  vec3 finalColor = col * grid * (u_bass + 0.2);
  finalColor += vec3(1.0, 0.9, 1.0) * (0.01 / (r + 0.002)) * u_level;
...
void main() {
  ...
  float numRays = 12.0 * u_zoom;
  float pattern = sin(a * numRays + t * 2.0) * 0.1 * u_mid;
  float d = abs(r - 0.3 - pattern);
  float glow = 0.008 / (d + 0.005);
  
  vec3 col = mix(vec3(1.0, 0.3, 0.0), vec3(0.1, 0.9, 1.0), sin(a * 2.0) * 0.5 + 0.5) * glow;
  col += vec3(1.0, 0.95, 0.8) * (0.02 / (r + 0.005)) * (u_bass + 0.5);
...
void main() {
  vec2 uv = (gl_FragCoord.xy - 0.5 * u_resolution) / min(u_resolution.x, u_resolution.y);
  float r = length(uv);
  float angle = atan(uv.y, uv.x) + r * u_zoom - u_time * u_speed * 0.2 - u_bass * 0.3;
  vec2 rotUv = vec2(r * cos(angle), r * sin(angle));
  
  float f1 = sin(rotUv.x * 3.0 + u_time * u_speed * 0.5) * cos(rotUv.y * 3.0 - u_time * u_speed * 0.3);
  float f2 = sin(rotUv.y * 6.0 - u_time * u_speed) * cos(rotUv.x * 6.0 + u_time * u_speed * 0.8) * u_mid;
  float combined = f1 + 0.5 * f2;
  
  vec3 colA = vec3(0.8, 0.1, 0.4);
  vec3 colB = vec3(0.1, 0.2, 0.8);
  vec3 colC = vec3(0.9, 0.8, 0.1);
  
  vec3 nebula = mix(colB, colA, combined * 0.5 + 0.5);
  nebula += colC * max(0.0, combined * combined) * u_treble * 0.5;
  
  float center = 0.08 / (r + 0.02) * (1.0 + u_bass * 0.5);
  vec3 finalColor = nebula * (1.0 - r) + center * vec3(0.9, 0.6, 0.9);
...
void main() {
  ...
  float grid = 0.2 / u_zoom;
  vec2 cell = floor(uv / grid);
  vec2 pos = uv - cell * grid;
  
  float height = 0.1 + sin(cell.x * 0.5 + u_time * u_speed * 0.2) * 0.2;
  height += sin(cell.y * 0.5 + u_time * u_speed * 0.3) * 0.2;
  height += u_bass * 0.5;
  
  float windowY = mod(pos.y * (20.0 * u_zoom), 1.0);
  float windowX = mod(pos.x * (20.0 * u_zoom), 1.0);
  ...
  float blink = sin(u_time * u_speed * 10.0 + cell.x * 10.0 + cell.y * 10.0);
...
void main() {
  ...
  float n = noise(uv0 * (5.0 / u_zoom) + t * 0.5);
  n += noise(uv0 * (10.0 / u_zoom) - t) * 0.5;
  n = pow(n, 2.0);
...
void main() {
  ...
  float t = u_time * u_speed * 0.5;
  float wave1 = sin(uv0.x * (5.0 * u_zoom) - t * 2.0) * 0.5;
  wave1 += sin(uv0.x * (10.0 * u_zoom) - t * 3.0 + uv0.y * 2.0) * 0.3;
  float wave2 = sin(uv0.x * (3.0 * u_zoom) + t * 1.5) * 0.4;
  wave2 += sin(uv0.x * (7.0 * u_zoom) + t * 2.0 + uv0.y * 1.5) * 0.2;
...
void main() {
  ...
  float wave = sin(angle * (20.0 * u_zoom) - u_time * u_speed * 5.0 + radius * 10.0);
  wave += sin(angle * (40.0 * u_zoom) - u_time * u_speed * 10.0 + radius * 20.0) * 0.5;
